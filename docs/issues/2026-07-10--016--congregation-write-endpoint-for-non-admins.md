...
- [ ] `PATCH /congregations/{tenant_id}` — nazwa, opis, status; autoryzacja przez `verify_tenant_access` (docelowo `church.edit` z `PermissionService`)
- [ ] Ograniczyć zmianę `status` do wartości dozwolonych dla nie-admina (bez `published` bez weryfikacji?) — **do decyzji produktowej**
- [ ] `congregationApiService.updateCongregation()` → nowy endpoint; `/admin/tenants` zostaje dla panelu admina
- [ ] Zwrócić `role` w odpowiedzi listy zborów dla zalogowanego użytkownika (albo osobny `GET /congregations/mine`)
- [ ] `canManageCongregation()` oparte o realne pole
- [ ] Test integracyjny: pastor zapisuje nazwę własnego zboru (200), cudzego (403)

## Acceptance criteria

- Pastor widzi „Edytuj” przy swoim zborze i zapisuje wszystkie sekcje strony edycji
- Pastor nie widzi i nie może edytować cudzego zboru
- Admin zachowuje dotychczasowe możliwości
