...
def parse_address(text: str) -> dict[str, str | None]:
    ...
    if postal_match:
        ...
        if len(parts) > 1:
            ...
            city_match = re.match(r'^([A-ZĄĆĘŁŃÓŚŹŻ][a-ząćęłńóśźż]+(?:\s+[A-ZĄĆĘŁŃÓŚŹŻ][a-ząćęłńóśźż]+)*)', city_part)
            if city_match:
        ...
        if element.name == 'hr':
            ...
            for line in lines:
                ...
                if 'pastor:' in line.lower():
                    ...
                    pastor_match = re.search(r'Pastor:\s*(.+?)(?:\s+Telefon|\s+E-mail|$)', line, re.IGNORECASE)
                    if pastor_match:
                ...
                if 'http' in line.lower() or re.search(r'\b[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]{0,61}[a-zA-Z0-9])?)*\.[a-zA-Z]{2,}\b', line):
