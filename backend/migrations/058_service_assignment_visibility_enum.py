...
async def upgrade() -> None:
    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(
            text("ALTER TABLE service_assignments DROP COLUMN IF EXISTS show_on_card")
        )
        await conn.execute(
            text("ALTER TABLE service_assignments DROP COLUMN IF EXISTS phone_public")
        )
        await conn.execute(
            text("ALTER TABLE service_assignments DROP COLUMN IF EXISTS email_public")
        )
...
async def downgrade() -> None:
    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(
            text(
                "ALTER TABLE service_assignments DROP CONSTRAINT IF EXISTS ck_service_assignments_email_visibility"
            )
        )
        await conn.execute(
            text(
                "ALTER TABLE service_assignments DROP CONSTRAINT IF EXISTS ck_service_assignments_phone_visibility"
            )
        )
        await conn.execute(
            text(
                "ALTER TABLE service_assignments DROP CONSTRAINT IF EXISTS ck_service_assignments_card_visibility"
            )
        )
        await conn.execute(
            text(
                "ALTER TABLE service_assignments DROP COLUMN IF EXISTS email_visibility"
            )
        )
        await conn.execute(
            text(
                "ALTER TABLE service_assignments DROP COLUMN IF EXISTS phone_visibility"
            )
        )
        await conn.execute(
            text(
                "ALTER TABLE service_assignments DROP COLUMN IF EXISTS card_visibility"
            )
        )
